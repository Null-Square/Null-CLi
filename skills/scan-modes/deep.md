...
6. Cross-check findings against the selected compliance framework and generate readiness mapping.
7. Finish with an executive summary, exploit-path notes, affected surface, and explicit limitations.

Deep mode uses more agent steps for coverage. It still avoids brute force, credential
attacks, destructive payloads, persistence, and out-of-scope host discovery. This public
skill intentionally excludes advanced validation and exploit-chaining strategy, which live
only in the managed NullSquare platform.
